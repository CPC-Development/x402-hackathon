...
ENV_NOMINATIM_PORT=${NOMINATIM_PORT:-}
...
if [ -z "${ENV_NOMINATIM_PORT}" ]; then
  NOMINATIM_PORT=$(read_env_value NOMINATIM_PORT)
else
  NOMINATIM_PORT=${ENV_NOMINATIM_PORT}
fi
...
if [ -z "${NOMINATIM_PORT:-}" ]; then
  NOMINATIM_PORT=$((NOMINATIM_BASE + OFFSET))
fi
